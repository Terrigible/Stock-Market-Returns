...
def calculate_lumpsum_portfolio_value_with_fees_and_interest_vector(
    ...
    *,
    ...
    dca_weights = pd.RangeIndex(dca_length).to_series().mod(dca_interval).eq(0).mul(1)

    remaining_capital_multiplier = (
        pd.RangeIndex(dca_length)
        .to_series()
        .floordiv(dca_interval)
        .rsub(np.ceil(dca_length / dca_interval))
    )
        ...
        .rdiv(
            (
                ...
                * (1 - variable_transaction_fees)
                / np.ceil(dca_length / dca_interval)
            )
